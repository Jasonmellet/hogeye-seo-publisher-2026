#!/usr/bin/env python3
"""
Fix Water Sports Page - Complete Rebuild
Rebuilds from source JSON with all H2 sections, proper formatting, and CTA.
"""
...
def load_source_content():
    """Load content from source JSON file, handling parsing errors."""
    try:
        with open('content/pages/water-sports-update.json', 'r') as f:
            return json.load(f)
    except json.JSONDecodeError:
        # Try reading as text and extracting content field - handle multiline
        with open('content/pages/water-sports-update.json', 'r') as f:
            file_content = f.read()
            ...
            while end_idx < len(file_content) and not found_end:
                if file_content[end_idx] == '"':
                    ...
                    # Look ahead to see if next non-whitespace is comma or closing brace
...
def main():
    console.print(Panel.fit(
        "[bold cyan]Fix Water Sports Page - Complete Rebuild[/bold cyan]",
        border_style="cyan"
    ))
    ...
    if update_response.status_code == 200:
        ...
        console.print(Panel.fit(
            ...
            f"Rebuilt Water Sports page with:\n"
