#!/usr/bin/env python3
"""
Update Events & Night Activities Landing Page
...
PAGE_ID = 1980
PAGE_TITLE = "Events & Night Activities"
...
def add_hero_section(content: str, hero_image_url: str = '') -> str:
    ...
    # Default hero image if none provided (use a placeholder or find one)
    if not hero_image_url:
        # We'll use a generic camp image or let theme handle it
        hero_image_url = 'https://www.camplakota.com/wp-content/uploads/2024/01/camp-evening-activities.jpg'
    
    hero_html = f'''<div class="hero-section" style="position: relative; margin-bottom: 3rem;">
  <img src="{hero_image_url}" alt="Camp Lakota Events and Night Activities" style="width: 100%; height: auto; display: block; max-height: 500px; object-fit: cover; border-radius: 8px;">
...
def add_images_to_content(content: str, image_ids: list, session) -> str:
    ...
    if len(re.findall(r'<img[^>]*class="[^"]*events-night-image', content)) >= 2:
...
<div class="large-image-block" style="padding: 1rem 0; margin: 2rem 0;">
  <img src="{image_url}" alt="Camp Lakota evening activities" class="events-night-image-1" style="width: 100%; max-height: 500px; object-fit: cover; border-radius: 8px; display: block;">
</div>
'''
...
<div class="side-by-side-images" style="display: flex; gap: 1rem; margin: 2rem 0; flex-wrap: wrap;">
  <div style="flex: 1; min-width: 300px;">
    <img src="{img1_url}" alt="Camp Lakota night activities" class="events-night-image-2" style="width: 100%; height: auto; border-radius: 8px;">
  </div>
  <div style="flex: 1; min-width: 300px;">
    <img src="{img2_url}" alt="Camp Lakota evening events" class="events-night-image-3" style="width: 100%; height: auto; border-radius: 8px;">
  </div>
</div>
'''
...
def main():
    ...
    console.print("[cyan]Searching for relevant images...[/cyan]")
    image_ids = []
    
    # Try to find images related to events/night activities
    search_terms = ['event', 'night', 'campfire', 'evening', 'dance', 'talent']
    ...
    if not image_ids:
        ...
        console.print("[yellow]No specific images found, will use placeholder[/yellow]")
        image_ids = [2626, 2581, 1280]  # Fallback images
